    ...
    <header className="top-nav">
      ...
      {loggedIn && (
        <nav className={mobileOpen ? "top-nav-center open" : "top-nav-center"} aria-label="Primary navigation">
          <button className="nav-text-button" onClick={() => window.dispatchEvent(new Event("open-command"))}>Search</button>
          <span className="nav-divider" />
          <NavDropdown label="Repos">
          ...
          <>
            ...
            <div className="dropdown click-dropdown">
              <button className="avatar-trigger" onClick={() => setAvatarOpen(!avatarOpen)} aria-label="Account menu">
                {profile.avatarUrl ? <img src={profile.avatarUrl} alt="" /> : <span>{profile.initials || initials}</span>}
                <ChevronDown size={14} />
              </button>
              {avatarOpen && (
