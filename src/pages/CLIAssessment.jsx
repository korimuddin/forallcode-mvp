...
import { CLI_QUESTIONS } from "../data/certQuestions/cliQuestions";
...
const passMark = 70;
const certPath = "/certification/command-line-essentials";

export default function CLIAssessment() {
  useDocumentTitle("Command Line Essentials Assessment");
  ...
  const [answers, setAnswers] = useState(Array(CLI_QUESTIONS.length).fill(null));
  ...
  async function handleSubmit() {
    ...
    const score = submittedAnswers.reduce((acc, answer, questionIndex) => (
      acc + (answer === CLI_QUESTIONS[questionIndex].correct ? 1 : 0)
    ), 0);
    const percentage = Math.round((score / CLI_QUESTIONS.length) * 100);
    ...
    navigate(`/certification/command-line-essentials/result?score=${percentage}&passed=${passed ? "true" : "false"}`);
  }
  ...
  const question = CLI_QUESTIONS[index];
  const progress = ((index + 1) / CLI_QUESTIONS.length) * 100;
  const sections = useMemo(() => [...new Set(CLI_QUESTIONS.map((item) => item.section))], []);
    ...
    return (
      <div className="cert-page">
        <section className="cert-card">
          ...
          <p>Buy the Command Line Essentials Certificate before starting the assessment.</p>
  ...
  return (
    <div className="cert-page">
      <div className="cert-assessment-layout">
        <aside className="cert-section-sidebar cli-section-sidebar">
          <p className="eyebrow">Sections</p>
          {sections.map((section, sectionIndex) => {
            const state = sectionIndex < currentSectionIndex ? "complete" : sectionIndex === currentSectionIndex ? "active" : "";
            return (
              <button className={state} key={section} onClick={() => setIndex(CLI_QUESTIONS.findIndex((item) => item.section === section))} type="button">
                <i />
                <span>{section}</span>
              </button>
            );
        ...
        <section className="cert-assessment cli-assessment">
          <header className="cert-assessment-header">
            <div>
              <p className="eyebrow">Question {index + 1} of {CLI_QUESTIONS.length}</p>
              <h1>Command Line Essentials Assessment</h1>
            </div>
          ...
          <div className="cert-question-text">{formatQuestionText(question.question)}</div>
