...
import { GIT_TEAMS_QUESTIONS } from "../data/certQuestions/gitTeamsQuestions";
...
export default function GitTeamsAssessment() {
  ...
  const [answers, setAnswers] = useState(Array(GIT_TEAMS_QUESTIONS.length).fill(null));
  ...
  async function handleSubmit() {
    ...
    const score = submittedAnswers.reduce((acc, answer, questionIndex) => (
      acc + (answer === GIT_TEAMS_QUESTIONS[questionIndex].correct ? 1 : 0)
    ), 0);
    const percentage = Math.round((score / GIT_TEAMS_QUESTIONS.length) * 100);
    ...
    navigate(`/certification/git-for-teams/result?score=${percentage}&passed=${passed ? "true" : "false"}`);
  }
  ...
  const sections = useMemo(() => [...new Set(GIT_TEAMS_QUESTIONS.map((item) => item.section))], []);
